...
class Instant_Articles_AMP_Markup
{
    ...
    static function markup_version()
    {
        ...
        $has_stylesheet = isset($settings[self::SETTING_STYLE])
            ? self::validate_json($settings[self::SETTING_STYLE])
            : false;

        $properties = [];

        $download_media = isset($settings[self::SETTING_DL_MEDIA])
            ? (bool) $settings[self::SETTING_DL_MEDIA]
            : false;

        $properties[
            AMPArticle::ENABLE_DOWNLOAD_FOR_MEDIA_SIZING_KEY
        ] = $download_media;

        if ($has_stylesheet) {
            $properties[AMPArticle::OVERRIDE_STYLES_KEY] = json_decode(
                $settings[self::SETTING_STYLE],
                true
            );
        }
