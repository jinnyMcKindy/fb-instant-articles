...
$results = $wpdb->get_results(" SELECT id_post FROM  $table_name ");
$ids = [];
foreach ($results as $page) {
    array_push($ids, (int) $page->id_post);
}
//var_dump($ids);
header(
    'Content-Type: ' .
...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/">
	<channel>
		<title> <?php bloginfo_rss('name'); ?> - <?php esc_html_e(
     'Instant Articles',
     'instant-articles'
 ); ?></title>
  ...
  if (
      false ===
      apply_filters(
          'instant_articles_should_submit_post',
          true,
          $instant_article_post
      )
  ) {
      continue;
  }
  ...
  if (
      ...
      !strlen(trim($instant_article_post->get_the_content()))
  ) {
      continue;
  }
  ...
  ?>
			<item>
				...
				<description><![CDATA[<?php echo esc_html(
        $instant_article_post->get_the_excerpt()
    ); ?>]]></description>
				<pubDate><?php echo esc_html(
        $instant_article_post->get_the_pubdate_iso()
    ); ?></pubDate>
				<modDate><?php echo esc_html(
        $instant_article_post->get_the_moddate_iso()
    ); ?></modDate>
