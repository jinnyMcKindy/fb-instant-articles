...
class Instant_Articles_Post
{
    ...
    public function get_canonical_url()
    {
        ...
        if (
            ...
            $post_clone->post_name = sanitize_title(
                $post_clone->post_name
                    ? $post_clone->post_name
                    : $post_clone->post_title,
                $post_clone->ID
            );
    ...
    protected function _get_the_content()
    {
        ...
        $cache_mod_time = get_transient(
            'instantarticles_mod_' . $this->_post->ID
        );
        if (
            get_post_modified_time('Y-m-d H:i:s', true, $this->_post->ID) ===
            $cache_mod_time
        ) {
            $content = get_transient(
                'instantarticles_content_' . $this->_post->ID
            );
    ...
    {
        $published_date = mysql2date(
            'c',
            get_post_time('Y-m-d H:i:s', true, $this->_post->ID),
            false
        );
         ...
         * @param Instant_Article_Post   $instant_article_post  The instant article post.
         */
        $published_date = apply_filters(
            'instant_articles_published_date_iso',
            $published_date,
            $this
        );
    ...
    public function get_the_moddate_iso()
    {
        ...
        $modified_date = apply_filters(
            'instant_articles_modified_date_iso',
            $modified_date,
            $this
        );
    ...
    public function get_the_featured_image()
    {
        ...
        if (has_post_thumbnail($this->_post->ID)) {
            $image_array = wp_get_attachment_image_src(
                get_post_thumbnail_id($this->_post->ID),
                'full'
            );
        ...
        $image_data = apply_filters(
            'instant_articles_featured_image',
            $image_data,
            $this->_post->ID
        );
        return $image_data;
    ...
    public function get_cover_media()
    {
        ...
        if (has_filter('instant_articles_cover_media')) {
             ...
             */
            $cover_media = apply_filters(
                'instant_articles_cover_media',
                $cover_media,
                $this->_post->ID
            );
        } else {
            ...
            if (
                isset($featured_image_data['src']) &&
                strlen($featured_image_data['src'])
            ) {
                $cover_media = Image::create()->withURL(
                    $featured_image_data['src']
                );
                if (
                    isset($featured_image_data['caption']) &&
                    strlen($featured_image_data['caption'])
                ) {
                    $cover_media->withCaption(
                        Caption::create()->withTitle(
    ...
    public function get_the_kicker()
    {
        ...
        if (has_category()) {
            ...
            if (
                is_array($categories) &&
                isset($categories[0]->name) &&
                __('Uncategorized') !== $categories[0]->name
            ) {
         ...
         * @param int     $post_id   The post ID.
         */
        $category_kicker = apply_filters(
            'instant_articles_cover_kicker',
            $category,
            $this->_post->ID
        );
    ...
    public function to_instant_article()
    {
        ...
        $date_time_zone = get_option('timezone_string')
        ...
        $file_path = apply_filters(
            'instant_articles_transformer_rules_configuration_json_file_path',
            $file_path
        );
        ...
        $transformer = apply_filters(
            'instant_articles_transformer_rules_loaded',
            $transformer
        );
        ...
        $transformer = apply_filters(
            'instant_articles_transformer_custom_rules_loaded',
            $transformer
        );
        ...
        $header = Header::create()
            ->withPublishTime(
                Time::create(Time::PUBLISHED)->withDatetime(
                    new DateTime($this->_post->post_date, $date_time_zone)
                )
            )
            ->withModifyTime(
                Time::create(Time::MODIFIED)->withDatetime(
                    new DateTime($this->_post->post_modified, $date_time_zone)
                )
            );
        ...
        $this->instant_article = InstantArticle::create()
            ...
            ->withHeader($header)
            ->addMetaProperty(
                'op:generator:application',
                'facebook-instant-articles-wp'
            )
            ->addMetaProperty(
                'op:generator:application:version',
                IA_PLUGIN_VERSION
            );
        ...
        if (isset($settings_publishing['likes_on_media'])) {
            Image::setDefaultLikeEnabled(
                $settings_publishing['likes_on_media']
            );
            Video::setDefaultLikeEnabled(
                $settings_publishing['likes_on_media']
            );
        }
        if (isset($settings_publishing['comments_on_media'])) {
            Image::setDefaultCommentEnabled(
                $settings_publishing['comments_on_media']
            );
            Video::setDefaultCommentEnabled(
                $settings_publishing['comments_on_media']
            );
        }
        ...
        $this->instant_article = apply_filters(
            'instant_articles_transformed_element',
            $this->instant_article
        );
    ...
    public function add_ads_from_settings()
    {
        ...
        $dimensions_match = [];
        $dimensions_raw = isset($settings_ads['dimensions'])
            ? $settings_ads['dimensions']
            : null;
        if (
            preg_match(
                '/^(?:\s)*(\d+)x(\d+)(?:\s)*$/',
        ...
        $source_of_ad = isset($settings_ads['ad_source'])
            ? $settings_ads['ad_source']
            : 'none';
        switch ($source_of_ad) {
            case 'none':
                break;
            ...
            case 'embed':
                if (!empty($settings_ads['embed_code'])) {
                    ...
                    $valid_html = @$fragment->appendXML(
                        $settings_ads['embed_code']
                    );
            ...
            default:
                if (!empty($source_of_ad)) {
                    ...
                    foreach (
                        $registered_compat_ads
                        as $compat_id => $compat_info
                    ) {
                        if (
                            array_key_exists($compat_id, $registered_compat_ads)
                        ) {
                            $document = new DOMDocument();
                            $fragment = $document->createDocumentFragment();
                            $valid_html = @$fragment->appendXML(
    ...
    public function add_analytics_from_settings()
    {
        ...
        if (
            isset($settings_analytics['embed_code_enabled']) &&
            !empty($settings_analytics['embed_code'])
        ) {
            $document = new DOMDocument();
            $fragment = $document->createDocumentFragment();
            $valid_html = @$fragment->appendXML(
                $settings_analytics['embed_code']
            );

            if ($valid_html) {
                $this->instant_article->addChild(
                    Analytics::create()->withHTML($fragment)
                );
            }
        }
        ...
        if (!empty($settings_analytics['integrations'])) {
            ...
            $registered_compat_analytics = Instant_Articles_Option::get_registered_compat(
    ...
    public function is_empty_after_transformation()
    {
        ...
        $cache = get_post_meta(
            $this->get_the_id(),
            '_is_empty_after_transformation',
            true
        );
        if ($cache) {
        ...
        if (
            count($instant_article->getChildren()) === 0 ||
            !$instant_article->getHeader() ||
            !$instant_article->getHeader()->getTitle()
        ) {
            update_post_meta(
                $this->get_the_id(),
                '_is_empty_after_transformation',
    ...
    public function should_submit_post()
    {
        ...
        // Only process posts
        $post_types = apply_filters('instant_articles_post_types', [
            'post',
            'paid_posts',
        ]);
        ...
        if (
            false ===
            apply_filters('instant_articles_should_submit_post', true, $this)
        ) {
            return false;
        }
        ...
        $force_submit = get_post_meta(
            $post->ID,
            IA_PLUGIN_FORCE_SUBMIT_KEY,
            true
        );
        if (
            $this->has_warnings_after_transformation() &&
            (!isset($publishing_settings['publish_with_warnings']) ||
                !$publishing_settings['publish_with_warnings']) &&
            !$force_submit
        ) {
            return false;
        }
    ...
    public function set_appearance_from_settings()
    {
        ...
        if (
            isset($settings['article_style']) &&
            !empty($settings['article_style'])
        ) {
    ...
    public function get_article_style()
    {
         ...
         * @param Instant_Article_Post      $instant_article_post   The instant article post.
         */
        $article_style = apply_filters(
            'instant_articles_style',
            'default',
            $this
        );
