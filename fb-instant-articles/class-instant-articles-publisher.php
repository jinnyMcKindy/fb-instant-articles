...
class Instant_Articles_Publisher
{
    ...
    public static function submit_article($post_id, $post)
    {
        ...
        if (
            false ===
            apply_filters('instant_articles_should_submit_post', true, $adapter)
        ) {
            return;
        }
        ...
        if (
            count($article->getChildren()) === 0 ||
            !$article->getHeader() ||
            !$article->getHeader()->getTitle()
        ) {
            return;
        }
        ...
        try {
            ...
            if (
                ...
                if (
                    count($adapter->transformer->getWarnings()) > 0 &&
                ...
                if ($dev_mode) {
                    ...
                    $published = apply_filters(
                        'instant_articles_post_published',
                        true,
                        $post_id
                    );
                }
                ...
                try {
                    // Import the article.
                    $submission_id = $client->importArticle(
                        $article,
                        $published
                    );
                    update_post_meta(
                        $post_id,
                        self::SUBMISSION_ID_KEY,
                        $submission_id
                    );
                } catch (Exception $e) {
