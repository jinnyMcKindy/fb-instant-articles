...
function show_version_incompatible_warning()
{
    echo '<div class="error"><p>' .
        esc_html__(
            'Instant Articles for WP requires PHP 5.4 to function properly. Please upgrade PHP or deactivate Instant Articles for WP.',
            'instant-articles'
        ) .
        '</p></div>';
}
if (version_compare(PHP_VERSION, '5.4', '<')) {
    add_action('admin_notices', 'show_version_incompatible_warning');
    return;
} else {
    ...
    $autoloader->add(
        'Facebook\\',
        __DIR__ .
            '/vendor/facebook/facebook-instant-articles-sdk-extensions-in-php/src'
    );
    ...
    function instant_articles_setup_admin_notice()
    {
        ...
        if (
            $pagenow === 'plugins.php' &&
            !Instant_Articles_Option_FB_Page::get_option_decoded()["page_id"]
        ) {
            $settings_url = Instant_Articles_Wizard::get_url();
            echo '<div class="updated settings-error notice is-dismissible">';
            echo '<p>Congrats, you\'ve installed the Instant Articles for WP plugin. Now <a href="' .
                esc_url_raw($settings_url) .
                '">set it up</a> to start publishing Instant Articles.';
            echo '</div>';
        }
    }
    ...
    /* Creating extra table for pages we want to remove from RSS feed */

    global $jal_db_version;
    $jal_db_version = '1.0';
    function yu_createTable()
    {
        global $wpdb;
        global $jal_db_version;

        $table_name = $wpdb->prefix . 'instant_articles_ids';
    ...
    function instant_articles_query($query)
    {
        if ($query->is_main_query() && $query->is_feed(INSTANT_ARTICLES_SLUG)) {
             ...
             */
            if (
                defined('INSTANT_ARTICLES_LIMIT_POSTS') &&
                INSTANT_ARTICLES_LIMIT_POSTS
            ) {
    ...
    function inject_url_claiming_meta_tag()
    {
        ...
        if (isset($fb_page_settings['page_id'])) { ?>
			<meta property="fb:pages" content="<?php echo esc_attr(
    ...
    add_action('wp_head', ['Instant_Articles_AMP_Markup', 'inject_link_rel']);
    ...
    add_action('wp', ['Instant_Articles_AMP_Markup', 'markup_version']);
    ...
    function rescrape_article($post_id, $post)
    {
        ...
        if ($adapter->should_submit_post()) {
            try {
                $client = Facebook\HttpClients\HttpClientsFactory::createHttpClient(
                    null
                );
                ...
                foreach ($old_slugs as $slug) {
                    ...
                    $url_encoded = urlencode(
                        $clone_adapter->get_canonical_url()
                    );
                    $client->send(
                        "https://graph.facebook.com/?id=$url_encoded&scrape=true",
                        'POST',
...
/* Ajax handle for pages we want to remove from RSS feed */

add_action('wp_ajax_add_article', 'prefix_ajax_add_article');

function prefix_ajax_add_article()
{
    global $wpdb;
    $id = (int) $_POST["data"]["post_id"];
    $submit = $_POST["data"]["submit"];
    $table_name = $wpdb->prefix . 'instant_articles_ids';
    if ($submit === "true") {
        ...
        $wpdb->query("DELETE FROM $table_name WHERE id_post = $id");
...
add_action('wp_ajax_get_article', 'prefix_ajax_get_article');

function prefix_ajax_get_article()
{
    global $wpdb;
    $id = (int) $_POST["data"]["post_id"];
    ...
    $results = $wpdb->get_results(
        "SELECT * FROM $table_name WHERE id_post = $id"
    );
    if (!empty($results)) {
        echo "true";
    }
